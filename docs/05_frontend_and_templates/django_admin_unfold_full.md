> Повний матеріал відновлено з `archive/legacy_docs/docs_flat/DJANGO_ADMIN_UNFOLD.md`.
...
```python
# urls.py
from notes_app.admin_sites import operations_admin, analytics_admin
...
```html
{# templates/admin/notes_app/note/change_form.html #}
{% extends "admin/change_form.html" %}
{% block object-tools-items %}
{{ block.super }}
<li>
    <a href="{% url 'admin:notes_app_note_export' original.pk %}"
...
```
HTTP Request /admin/notes_app/note/42/change/
    │
    ▼
Middleware stack:
...
def response_change(self, request, obj):
    ...
    if '_continue' not in request.POST and '_addanother' not in request.POST:
        ...
        return redirect(reverse('admin:notes_app_note_changelist'))
...
- **📚 Чому це існує:** Ієрархія шаблонів дозволяє override на різних рівнях деталізації: хочеш змінити щось для ВСІХ моделей — правиш `templates/admin/change_list.html`. Хочеш змінити для всіх моделей конкретного додатку — правиш `templates/admin/notes_app/change_list.html`. Хочеш змінити тільки для `Note` — правиш `templates/admin/notes_app/note/change_list.html`. Django шукає від найспецифічнішого до найзагальнішого.
- **🌐 Що відбувається під капотом:** Django Template Engine при рендерингу адмін-шаблону запитує `TemplateEngine.select_template(['admin/notes_app/note/change_list.html', 'admin/notes_app/change_list.html', 'admin/change_list.html'])`. Він перевіряє файлову систему для кожного варіанту по черзі і повертає перший знайдений. Це відбувається при КОЖНОМУ запиті (або з кешу при `cached.Loader`).
...
└── admin/
    ...
    └── notes_app/
        ├── change_list.html      ← для всіх моделей notes_app
        └── note/
...
UNFOLD = {
    ...
    "SITE_URL": "/",
    "SITE_ICON": lambda request: static("notes_app/img/logo.png"),
    "SITE_LOGO": lambda request: static("notes_app/img/logo-full.png"),
    ...
    # ── Dashboard ─────────────────────────────────────────────
    "DASHBOARD_CALLBACK": "notes_app.admin_dashboard.dashboard_callback",
                ...
                "items": [
                    {
                        "title": "Всі нотатки",
                        "icon": "description",
                        "link": reverse_lazy("admin:notes_app_note_changelist"),
                        "badge": "notes_app.admin_callbacks.get_draft_count",
                        "badge_type": BadgeType.WARNING,
                    },
                    {
                        "title": "Категорії",
                        "icon": "label",
                        "link": reverse_lazy("admin:notes_app_category_changelist"),
                    },
                ],
            },
            {
    ...
    "TABS": [
        {
            "models": ["notes_app.note", "notes_app.comment"],
            "items": [
                {
                    "title": "Нотатки",
                    "icon": "description",
                ...
                },
                {
                    "title": "Коментарі",
                    "icon": "comment",
                    "link": reverse_lazy("admin:notes_app_comment_changelist"),
                },
            ],
        },
    ],
...
- **📚 Чому `reverse_lazy` а не `reverse`:** Sidebar конфігурація у `settings.py` виконується при завантаженні модуля — до того як Django повністю ініціалізував URL-маршрути. `reverse('admin:notes_app_note_changelist')` в цей момент викине `NoReverseMatch`. `reverse_lazy` — це "відкладений reverse", який виконується тільки при першому зверненні (вже після повної ініціалізації).
- **🌐 Що відбувається під капотом:** Sidebar badge callback (`"notes_app.admin_callbacks.get_draft_count"`) — це рядковий шлях до Python-функції. Unfold динамічно імпортує і викликає цю функцію при КОЖНОМУ рендерингу сайдбару (при кожному запиті). Якщо badge callback повільний (складний SQL) — він уповільнить кожну сторінку адмінки. Кешуй результат якщо можливо.
- **❌ Типова помилка початківця:** Писати `"link": reverse("admin:notes_app_note_changelist")` (без `_lazy`) у `settings.py` на рівні модуля — і отримувати `ImproperlyConfigured: The included URLconf ... does not appear to have any patterns`. Завжди `reverse_lazy` у `settings.py`, `UNFOLD` словнику, і будь-якому місці де URL генерується до повного завантаження Django.
---
...
"TABS": [
    {
        "models": ["notes_app.note", "notes_app.comment", "notes_app.category"],
        "items": [
            {
                "title": "Нотатки",
                "icon": "description",
    ...
    tabs = [
        {
            "title": "Мої нотатки",
            "icon": "person",
            "link": f"{reverse('admin:notes_app_note_changelist')}?author={request.user.pk}",
        },
    ]
    if request.user.is_superuser:
        tabs.append({
            "title": "Всі нотатки",
            "icon": "groups",
            "link": reverse("admin:notes_app_note_changelist"),
        })
    return tabs
```
...
"TABS": [
    {
        "models": ["notes_app.note"],
        "items": "notes_app.admin_callbacks.note_tabs_callback",
    },
],
```
...
- **🌐 Що відбувається під капотом:** `AutocompleteSelectFilter` під капотом використовує Django вбудований autocomplete API: `GET /admin/auth/user/autocomplete/?term=john&app_label=notes_app&model_name=note&field_name=author`. Django Admin виконує `search_fields` query на UserAdmin і повертає JSON з результатами. Тому `search_fields` на `UserAdmin` є ОБОВ'ЯЗКОВИМ для роботи `AutocompleteSelectFilter`.
...
class NoteAdmin(UnfoldModelAdmin):
    ...
    def duplicate_note(self, request, pk):
        ...
        return redirect(
            reverse('admin:notes_app_note_changelist')
        )
```
...
- **📚 Чому `apps.py → ready()` для реєстрації:** `@register_component` реєструє компонент у глобальний реєстр при імпорті модуля. Але Python не імпортує модуль автоматично — лише якщо він явно import-ований. `apps.py → ready()` — це правильне місце для "побічних ефектів при старті Django": він виконується після повного завантаження всіх моделей і налаштувань. `import notes_app.admin_components` у `ready()` ініціює реєстрацію.
- **🌐 Що відбувається під капотом:** `{% component "notes_app.admin_components.NoteActivityChartComponent" %}` — Unfold знаходить клас у реєстрі за рядковим шляхом, створює екземпляр, викликає `get_context_data()`, і рендерить вміст блоку компонента у цьому контексті. Вміст між `{% component %}` і `{% endcomponent %}` — це шаблон компонента, якому доступні дані з `get_context_data()`.
- **❌ Типова помилка початківця:** Забути `import notes_app.admin_components` у `apps.py → ready()`. Компоненти не будуть зареєстровані. `{% component "..." %}` тихо відобразить нічого або кине `ComponentNotFound`. Перевірка: після зупинки та перезапуску сервера — помилка `ComponentNotFound` → `ready()` не імпортує потрібний модуль.
---
...
class HelloAppConfig(AppConfig):
    ...
    def ready(self):
        import notes_app.admin_components  # ← реєстрація компонентів
```
...
<div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
    ...
    {% component "notes_app.admin_components.NoteActivityChartComponent" %}
    {% component "notes_app/components/chart/line.html" with data=chart_data title=title %}
    {% endcomponent %}
    {% endcomponent %}

    {% component "notes_app.admin_components.TopAuthorsTableComponent" %}
...
class NoteStatisticsView(UnfoldModelAdminViewMixin, TemplateView):
    ...
    permission_required = ()         # або ('notes_app.view_note',)
    template_name = 'admin/notes_app/note_statistics.html'
...
class NoteAdmin(UnfoldModelAdmin):
    ...
    def get_urls(self):
        from django.urls import path
        from notes_app.admin_views import NoteStatisticsView
        ...
        custom = [
            path(
                ...
                name='notes_app_note_statistics',
            ),
        ]
        return custom + urls
```
                ...
                "items": [
                    {
                        ...
                        "icon": "bar_chart",
                        "link": reverse_lazy("admin:notes_app_note_statistics"),
                    },
                ],
            },
        ],
...
```html
{# templates/admin/notes_app/note_statistics.html #}
{% extends "unfold/layouts/base.html" %}
{% load i18n unfold %}

{% block breadcrumbs %}
<a href="{% url 'admin:notes_app_note_changelist' %}">Нотатки</a> /
...
-- notes_app/migrations/0002_note_search_index.py
...
class Migration(migrations.Migration):
    operations = [
        migrations.RunSQL(
            "CREATE INDEX CONCURRENTLY idx_note_fts ON notes_app_note "
...
- **🌐 Що відбувається під капотом:** `comments_dataset_view` — це стандартний Django view, зареєстрований у `get_urls()`. Він виконується окремим HTTP-запитом (GET `/admin/notes_app/note/42/comments/?page=2`). `self.admin_site.each_context(request)` додає стандартні admin контекст змінні (права, sidebar, повідомлення), щоб partial шаблон виглядав як частина адмінки.
...
class NoteAdmin(UnfoldModelAdmin):
    ...
    def comments_dataset_view(self, request, note_id):
        ...
        return render(request, 'admin/notes_app/note/comments_dataset.html', {
            'note': note,
            'page_obj': page_obj,
