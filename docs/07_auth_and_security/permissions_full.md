> Повний матеріал відновлено з `archive/legacy_docs/docs_flat/PERMISSIONS.md`.
...
'notes_app.add_note'     # може створювати нотатки
'notes_app.view_note'    # може переглядати
'notes_app.change_note'  # може редагувати
'notes_app.delete_note'  # може видаляти

# Приклад використання:
from django.contrib.auth.decorators import permission_required

@permission_required('notes_app.delete_note')  # ← перевіряє дозвіл
...
**Проблема:** `@permission_required('notes_app.change_note')` перевіряє лише *"чи може цей юзер редагувати нотатки взагалі"*. Але не перевіряє *"чи належить ця конкретна нотатка йому"*.
...
У `crispy_notes_chat_app` ми використовуємо Django вбудовану модель `Group` **не для дозволів**, а для **спільного доступу до нотаток і списків покупок**.
