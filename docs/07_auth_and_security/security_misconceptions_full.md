> Повний матеріал відновлено з `archive/legacy_docs/docs_flat/SECURITY_MISCONCEPTIONS.md`.
...
```python
# Суперюзер:
user.has_perm('notes_app.delete_note')  # True — ЗАВЖДИ, навіть без явного права

# Звичайний staff-юзер:
user.has_perm('notes_app.delete_note')  # False — якщо право не призначено
