> Повний матеріал відновлено з `archive/legacy_docs/docs_flat/RELATIONAL_DB_FOUNDATIONS.md`.
...
>
> **🌐 Як Django реалізує це:** `ManyToManyField(Tag)` → Django автоматично створює таблицю `notes_app_note_tags` з двома FK. Ти ніколи не бачиш цю таблицю в Python, але вона є в БД. `note.tags.add(tag)` → INSERT в junction table.
>
...
class Note(models.Model):
    ...
    # Django автоматично створює: notes_app_note_tags (note_id FK, tag_id FK)
