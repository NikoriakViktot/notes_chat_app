> Повний матеріал відновлено з `archive/legacy_docs/docs_flat/DJANGO_NINJA_TEMPLATES.md`.
...
from notes_app.api import router as notes_router
api.add_router("/notes/", notes_router)

urlpatterns = [
    path('admin/', admin.site.urls),
    path('', include('notes_app.urls')),       # ← HTML views (SSR)
...
def note_list_lazy(request):
    ...
    return TemplateResponse(request, 'notes_app/note_list.html', context)
```
...
5. View (Ninja endpoint):
   ...
   └── render(request, 'note_list.html', context)
            │
6. Template Engine:
   ├── Locate: APP_DIRS=True → notes_app/templates/notes_app/note_list.html
...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <button
        ...
        hx-get="{% url 'notes_app:note_create_partial' %}"
...
<div id="notes-container">
    {% include 'notes_app/components/note_grid.html' %}
</div>
...
> Зверніть на ключову відмінність: `render(request, 'notes_app/components/note_grid.html', ...)` — тут шаблон є **фрагментом**, не повною сторінкою. У `note_grid.html` немає `{% extends %}`, немає `<html>`, немає `<head>` — тільки `<div>` з картками нотаток. Це те, що HTMX вставить у `#notes-container`.
...
def note_search(request, q: str = ""):
    ...
    return render(
        request,
        'notes_app/components/note_grid.html',  # ← фрагмент, не повна сторінка
        {'notes': notes, 'query': q}
    )
...
```html
{# notes_app/components/note_grid.html — ФРАГМЕНТ (без <!DOCTYPE>, без <html>) #}
{% if notes %}
...
```
render(request, 'notes_app/note_list.html', context)
         ↓
Django перебирає TEMPLATES список (порядок важливий!):
         ↓
Backend 1 (DjangoTemplates):
  Шукає 'notes_app/note_list.html' у templates/ директоріях
  → Знайшов? → Рендерить DTL → СТОП
  → Не знайшов? → Переходить до Backend 2

Backend 2 (Jinja2):
  Шукає 'notes_app/note_list.html' у jinja2/ директоріях
...
TEMPLATES = [
    ...
    },
    {
        ...
        'OPTIONS': {
            'environment': 'notes_app.jinja2_env.environment',
        },
    },
]
```
...
> в [ADVANCED_TEMPLATES.md §6](../05_frontend_and_templates/advanced_templates_full.md) — Jinja2 Deep Dive.
...
def note_list_jinja(request):
    ...
    return render(request, 'notes_app/note_list.html.j2', {'notes': notes})
```
...
def note_list(request):
    ...
    response = TemplateResponse(request, 'notes_app/note_list.html', context)
...
def good_view(request):
    return render(request, 'notes_app/note_list.html', {'notes': Note.objects.all()})
...
# ✅ Named URL
<a href="{% url 'notes_app:note_detail' pk=note.pk %}">...</a>


# ❌ Static URL hardcode
<img src="/static/notes_app/img/logo.png">
